...
srcprefix = "http://news.hit.edu.cn"
allImgUrl = []
...
def getContext(c_url):
	...
	for i, url in enumerate(c_url):
		try :
			print("handling %dth context from hit" %(i + 1))
			...
			ret.append('error')
			print("error when handling %dth context from hit" %(i + 1))
...
def main():
	...
	test = sqlConnect("hit", 10)
	tParser = titleParser()
	link = urllib2.urlopen("http://news.hit.edu.cn/xxyw/list.htm")
	srcCode = link.read()
