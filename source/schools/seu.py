...
srcprefix = "http://news.seu.edu.cn"
allImgUrl = []
...
def getContext(c_url):
	...
	for i, url in enumerate(c_url):
		try :
			print("handling %dth context from seu" %(i + 1))
			link = urllib2.urlopen(url)
			srcCode = link.read()
			newsPattern = '<div class=\'Article_Content\'>[\s\S]+?</div>'
			...
			ret.append('error')
			print("error when handling %dth context from seu" %(i + 1))
...
def main():
	...
	test = sqlConnect("seu", 10)
	tParser = titleParser()
	try:
		link = urllib2.urlopen("http://news.seu.edu.cn/5486/list.htm")
		srcCode = link.read()
		srcCode = re.compile(r'<div id="wp_news_w4">[\s\S]+?<div id="wp_paging_w4">').findall(srcCode)
		srcCode = srcCode[0]
