...
srcprefix = "http://zs.njust.edu.cn"
allImgUrl = []
...
def getContext(c_url):
	...
	for i, url in enumerate(c_url):
		try :
			print("handling %dth context from njust" %(i + 1))
			...
			ret.append('error')
			print("error when handling %dth context from njust" %(i + 1))
	return ret
...
def main():
	...
	test = sqlConnect("njust", 10)
	tParser = titleParser()
	link = urllib2.urlopen("http://zs.njust.edu.cn/3550/list.htm")
	srcCode = link.read()
