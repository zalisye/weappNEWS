...
srcprefix = "http://news.ahu.edu.cn"
allImgUrl = []
...
def getContext(c_url):
	...
	for i, url in enumerate(c_url):
		try :
			print("handling %dth context from ahu" %(i + 1))
			...
			newsblock = re.sub(r'</span>', '', newsblock)
			newsblock = re.sub(r'<strong>', '', newsblock)
			newsblock = re.sub(r'</strong>', '', newsblock)
			parser = titleParser()
			...
			ret.append('error')
			print("error when handling %dth context from ahu" %(i + 1))
...
def main():
	...
	test = sqlConnect("ahu", 10)
	tParser = titleParser()
	link = urllib2.urlopen("http://news.ahu.edu.cn/4642/list.htm")
	srcCode = link.read()
