...
srcprefix = "http://news.csu.edu.cn/"
imgprefix = "http://news.csu.edu.cn"
...
def mysqlToJson() :
	...
	cur = conn.cursor()
	cur.execute("SELECT * FROM `data` WHERE context <> 'error' AND abstract <> 'error' AND school = 'zhongnan' ORDER BY Time_stamp DESC")
	data = cur.fetchall()
	cur.close()
	jsonsrc = []
	jsonsrc.append(0)
	...
	path = (localpath if localflag == True else serverpath) + "json/zhongnan.json"
...
def getContext(c_url) :
	...
	for i, url in enumerate(c_url) :
		try :
			print "handling %dth context from zhongnan" %(i + 1)
			link = urllib2.urlopen(url)
			srcCode = link.read()
			imgPattern = r'<img width="\d+"\s*height="\d+"\s*src="([\s\S]*?)"'
			imgUrl = re.compile(imgPattern).findall(srcCode)
			if(len(imgUrl) == 0):
				allImgUrl.append("null")
			else:
				allImgUrl.append(imgUrl[0])
			contextPattern = r'<div class="subCont" style="line-height:25px;">[\s\S]+?</div>'
			...
			ret.append('error')
			print "error when handling %dth context from zhongnan" %(i + 1)
...
def getAbstract(allContext) :
	...
	for i, text in enumerate(allContext) :
		try :
			print "handling %dth abstract from zhongnan" %(i + 1)
			result = nlp.summary('', text, 50)
			ret.append(result.replace('\n', '').replace('\r', '').replace('\t', '').replace(r'\n', '').replace(r'\r','').replace(r'\t', ''))
		except :
			print "error when handling %dth abstract from zhongnan" %(i + 1)
			ret.append('error')
			print traceback.print_exc()
	return ret
...
def updateMySQL(tits, abss, txts, imgs, dts) :
	...
	for i in range(len(dts)) :
		...
		try :
			print "downloading %d.img from zhongnan" %maxid
			path = (localpath if localflag == True else serverpath) + ("img/zhongnan/%s.jpg" %maxid)
			urllib.urlretrieve(str(imgs[i]), path)
			sql = sql + "'http://www.vdebug.xyz/img/zhongnan/" + str(maxid) + ".jpg'" + ","
		except :
			print "error when downloading %d.img from zhongnan" %maxid
			sql = sql + "'error'" + ","
		try :
			print "downloading %d.mp3 from zhongnan" %maxid
			encodetext = urllib2.quote(abss[i].encode('utf8'))
			url = audioprefix + encodetext
			path = (localpath if localflag == True else serverpath) + ("audio/zhongnan/%s.mp3" %maxid)
			urllib.urlretrieve(url, path)
		except :
			print "error when downloading %d.mp3 from zhongnan" %maxid
		sql = sql + "'http://www.vdebug.xyz/audio/zhongnan/" + str(maxid) + ".mp3', 'zhongnan', DATE('" + str(dts[i]) + "')),"
	sql = sql[0 : len(sql) - 1]
	if(len(dts) > 0) :
		cur.execute(sql)
		conn.commit()
...
def getAvailableIndex(dts, tits) :
	...
	cur = conn.cursor()
	cur.execute("SELECT MAX(Time_stamp) FROM `data` WHERE School = 'zhongnan'")
	res = cur.fetchall()
	...
	for idx, dt in enumerate(dts) :
		...
		if(curdate > maxdate) :
			...
			sql = "SELECT * FROM `data` WHERE School = 'zhongnan' AND Time_stamp = " + "'" + str(curdate) + "' AND Title = '" + tits[idx] + "'"
			cur.execute(sql)
			res = cur.fetchall()
...
def main():
	...
	response = urllib2.urlopen("http://news.csu.edu.cn/xxyw.htm")
	srcCode=response.read()

	pattern = r'<DIV class="mainLeft mt8">(([\s\S]*))<DIV class="mainRit mt8">'
	srcCodeArray = re.compile(pattern).findall(srcCode.decode('utf-8'))

	datePattern = ur'<span>\s*\[(\d+)\\u5e74(\d+)\\u6708(\d+)\\u65e5\]\s*</span>'.decode('utf-8')
	allDate1 = re.compile(datePattern).findall(str(srcCodeArray[0]))[0 : cntlimit]
	allDate = [1]*len(allDate1) 
	for i in range(len(allDate1)):
		allDate[i] = allDate1[i][0]+'-'+allDate1[i][1]+'-'+allDate1[i][2]
